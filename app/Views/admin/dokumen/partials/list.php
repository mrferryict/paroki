<div id="dokumen-list" class="overflow-hidden rounded-lg border border-gold/20 bg-white shadow-sm">
    <div class="border-b border-gold/10 px-4 py-3">
        <h2 class="text-sm font-semibold uppercase tracking-wide text-stone-600">Daftar Unduhan</h2>
    </div>

    <?php if ($items === []): ?>
        <p class="px-4 py-8 text-center text-sm text-stone-500">Belum ada unduhan.</p>
    <?php else: ?>
                ...
                <li class="flex items-start gap-3 px-4 py-3">
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="mt-1 text-xs text-stone-500">
                            <?= esc($kategoriOptions[(string) $item->kategori] ?? (string) $item->kategori) ?>
                            · <?= esc(number_format((int) ($item->download_count ?? 0), 0, ',', '.')) ?> unduhan
                        </p>
                    </div>
                    <div class="flex shrink-0 flex-col gap-1">
                        ...
                        <button type="button" class="rounded border border-red-200 px-2 py-1 text-xs text-red-700 hover:bg-red-50"
                                hx-post="<?= site_url('admin/dokumen/' . $item->id . '/delete') ?>" hx-target="#dokumen-list" hx-swap="outerHTML" hx-confirm="Hapus unduhan ini?">Hapus</button>
                    </div>
                </li>
            <?php endforeach ?>
        </ul>

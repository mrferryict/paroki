<div id="katekese-kategori-list" class="overflow-hidden rounded-lg border border-gold/20 bg-white shadow-sm">
    ...
    <?php if ($items === []): ?>
        <p class="px-4 py-8 text-center text-sm text-stone-500">Belum ada kategori katekese.</p>
    <?php else: ?>
                ...
                <li class="flex items-start gap-3 px-4 py-3">
                    ...
                    <div class="flex shrink-0 gap-1">
                        <button type="button" class="rounded border border-maroon/20 px-2 py-1 text-xs text-maroon hover:bg-maroon/5"
                                hx-get="<?= site_url('admin/katekese-kategori/' . $item->id . '/edit') ?>"
                                hx-target="#katekese-kategori-form-panel" hx-swap="innerHTML">Edit</button>
                        <button type="button" class="rounded border border-red-200 px-2 py-1 text-xs text-red-700 hover:bg-red-50"
                                hx-post="<?= site_url('admin/katekese-kategori/' . $item->id . '/delete') ?>"
                                hx-target="#katekese-kategori-list" hx-swap="outerHTML"

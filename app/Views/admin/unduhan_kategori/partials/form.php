<?php /** @var \App\Entities\DokumenKategoriRecord|null $item */ ?>
...
<div>
    ...
    <form hx-post="<?= esc($action) ?>" hx-target="#unduhan-kategori-form-panel" hx-swap="innerHTML" class="space-y-4">
        <?= csrf_field() ?>
        ...
        <div>
            ...
            <p class="mt-1 text-xs text-stone-500">Kategori nonaktif tidak tampil di situs publik dan tidak bisa dipilih untuk unduhan baru.</p>
        </div>
        ...
        <div class="flex gap-2 pt-2">
            ...
            <button type="button" class="rounded border border-stone-300 px-4 py-2 text-sm hover:bg-stone-50"
                    onclick="document.getElementById('unduhan-kategori-form-panel').innerHTML = '<p class=\'text-sm text-stone-500\'>Form dibatalkan.</p>'">Batal</button>
        </div>
    </form>
</div>

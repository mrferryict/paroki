<?php /** @var \App\Entities\Wilayah|null $item */ ?>
<?php $isEdit = $item !== null && (int) ($item->id ?? 0) > 0; ?>
<div>
    <h2 class="mb-4 font-display text-xl font-semibold text-maroon"><?= $isEdit ? 'Edit Wilayah' : 'Tambah Wilayah' ?></h2>
    ...
    <form hx-post="<?= esc($action) ?>" hx-target="#wilayah-form-panel" hx-swap="innerHTML" class="space-y-4">
        ...
        <div>
            <label for="nama" class="mb-1 block text-sm font-medium">Nama Wilayah <span class="text-red-600">*</span></label>
